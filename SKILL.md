...
### superadmin 更新其他配置文件（Bot 自动提交）

superadmin 可通过对话修改 `config/` 下的其他文件（如 `access-control.md`），Bot 确认后自动调用 GitHub API 提交。流程与知识库更新相同：预览 → 确认 → 提交 → 刷新技能。

### superadmin 更新 SKILL.md / SOUL.md（手动）

SKILL.md 和 SOUL.md 是系统行为与安全规则的核心，**只能通过手动 git PR** 修改，Bot 不代为提交。提交时需同步：
...
admin 发出更新指令
  → Bot 收集内容并预览
    → admin 确认
      → Bot 调 GitHub API 提交到 $GITHUB_BASE_BRANCH
        → 对话中说「刷新技能」
          → 新知识库生效
```
...
Bot 收到后：
1. 展示将要新增的条目供 superadmin 确认
2. superadmin 确认后，Bot 调用 GitHub API 更新 `config/admins.yaml` 并提交到 `$GITHUB_BASE_BRANCH`
3. 告知 superadmin 说「刷新技能」使变更生效
...
> 提供任一平台 ID 即可定位。流程同上，superadmin 确认后 Bot 自动提交。
...
> **安全说明**：Bot 对 `config/admins.yaml` 的写入仅限于添加/移除 admin 这两个结构化操作，且需 superadmin 在对话中明确确认后才会执行。SOUL.md、SKILL.md 等系统文件的修改仍须手动 git PR。
